...
class TestDieComposition(unittest.TestCase):
    def test_string_rep(self):
        ...
        self.assertEqual(str(DieComposition("2 + 1d20 * 5")), "2 + 1 * d20 * 5")
        self.assertEqual(str(DieComposition("2(d5+3)")), "2 * ( d5 + 3 )")
        self.assertEqual(str(DieComposition("2d5+3d3")), "2 * d5 + 3 * d3")
        self.assertEqual(str(DieComposition("2*(2d14)/3")), "2 * ( 2 * d14 ) / 3")
        self.assertEqual(str(DieComposition("2*(5+(2d14+3)*2)/3")), "2 * ( 5 + ( 2 * d14 + 3 ) * 2 ) / 3")
    ...
    def test_eval_int(self):
        ...
        for pat, mi, ma in test_vector:
            ...
            for _ in range(repetitions):
                ...
                self.assertLessEqual(val, ma, f"pattern {pat} too big (limit {ma})")
                self.assertGreaterEqual(val, mi, f"pattern {pat} too small (limit {mi})")
                min_reached = min_reached or val == mi
                max_reached = max_reached or val == ma

            self.assertTrue(min_reached, f"Min value {mi} for pattern {pat} not hit in {repetitions} rounds")
            self.assertTrue(max_reached, f"Max value {ma} for pattern {pat} not hit in {repetitions} rounds")
